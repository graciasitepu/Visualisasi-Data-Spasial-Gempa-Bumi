...
<body>
    <header>
        ...
        <div>
            ...
            <p>Visualize Earthquake Data</p>
        </div>
    </header>

    <div id="container">
        <div id="map-area" style="width: 65%;"> <!-- Adjust the width here -->
                ...
                <table id="locations">
                    <thead>
                        <tr>
                            <th>Date</th>
                            <th>Mag</th>
                            <th>Location</th> <!-- Remove Depth column -->
                        </tr>
                    </thead>
                    <tbody>
    ...
    <script>
        var map = L.map('map').setView([-3.507, 119.666], 5);
        ...
        $sql = "SELECT ot, lat, lon, mag, dept, loc FROM gempa";
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "var circle = L.circle([" . $row['lat'] . ", " . $row['lon'] . "], {
                    ...
                    radius: " . 20000 . ",
                    time: '" . $row['ot'] . "'
                }).addTo(map).bindPopup('Magnitude: " . $row['mag'] . "<br>Depth: " . $row['dept'] . "<br>Location: " . $row['loc'] . " km');";
        ...
        document.getElementById('magnitude-select').addEventListener('change', function() {
            ...
            filterCirclesByMagnitudeRange(selectedMagnitudeRange);
        });
